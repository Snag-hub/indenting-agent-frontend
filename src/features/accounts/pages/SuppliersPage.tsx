...
import { supplierApi, type SupplierDto } from '@/features/accounts/api/supplierApi'
import { SupplierFormDialog } from '@/features/accounts/components/SupplierFormDialog'
...
export function SuppliersPage() {
  ...
  const [editing, setEditing] = useState<SupplierDto | undefined>()
  const [deleting, setDeleting] = useState<SupplierDto | undefined>()

  const { data, isLoading } = useQuery({
    queryKey: queryKeys.suppliers.list({ search, page }),
    queryFn: () => supplierApi.list({ search, page, pageSize: 20 }),
  })
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2 justify-end">
          <Button size="icon" variant="ghost" asChild>
            <Link to="/suppliers/$id" params={{ id: row.original.id }}>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Suppliers"
        description="Manage supplier accounts and item catalogs."
        action={
